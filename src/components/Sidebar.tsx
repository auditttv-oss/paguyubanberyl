...
export const Sidebar: React.FC<SidebarProps> = ({ onCloseMobile }) => {
  const { signOut, user, isSecurity } = useAuth();

  // FILTER MENU UTAMA BERDASARKAN STATUS KEAMANAN/SATPAM
  const navItems = isSecurity 
    ? [
        { to: '/keamanan', label: 'Log Keamanan', icon: ShieldAlert, protected: true },
        { to: '/fasum', label: 'Jadwal Fasum', icon: CalendarDays, protected: false }
      ]
    : [
        ...
        { to: '/keamanan', label: 'Log Keamanan', icon: ShieldAlert, protected: true },
        { to: '/fasum', label: 'Jadwal Fasum', icon: CalendarDays, protected: false } // Terbuka untuk umum
      ];
  ...
  return (
    <div className="flex flex-col h-full bg-gradient-to-b from-emerald-900 to-teal-900 text-white shadow-xl">
      ...
      <div className="p-6 border-b border-emerald-800/50">
        ...
        <div className={`mt-6 text-xs px-3 py-1.5 rounded-lg inline-flex items-center gap-2 font-bold w-full ${
          user 
            ? isSecurity 
              ? 'bg-amber-500/20 text-amber-200 border border-amber-500/30'
              : 'bg-emerald-600/50 text-white border border-emerald-500/50' 
            : 'bg-amber-500/20 text-amber-200 border border-amber-500/30'
        }`}>
          <div className={`w-2 h-2 rounded-full ${user ? isSecurity ? 'bg-amber-400' : 'bg-emerald-400' : 'bg-amber-400'} animate-pulse`}></div>
          {user ? isSecurity ? 'Mode Satpam Aktif' : 'Mode Admin Aktif' : 'Mode Tamu (View Only)'}
        </div>
      </div>
      ...
      <div className="p-4 border-t border-emerald-800 bg-emerald-900/30">
        ...
        <p className="text-[10px] text-center text-emerald-400/60 mt-4 font-mono">
          v2.6 • Cluster Beryl
        </p>
      </div>
    </div>
