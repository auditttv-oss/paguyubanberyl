        ...
        <section className="mb-6 md:mb-8">
          <div className="bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-gray-200">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h3 className="text-lg font-bold text-gray-800">Partisipasi Warga</h3>
                <p className="text-gray-600 text-sm">Kepatuhan kas wajib dan sukarela</p>
              </div>
            </div>
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsPieChart>
                  <Pie
                    data={[
                      { name: 'Sudah Kas', value: stats.paidKas, color: '#10b981' },
                      { name: 'Belum Kas', value: stats.unpaidKas, color: '#ef4444' },
                      { name: 'Sudah Sukarela', value: stats.donatedSukarela, color: '#3b82f6' },
                      { name: 'Belum Sukarela', value: stats.notDonatedSukarela, color: '#f59e0b' }
                  ...
                  >
                    {[
                      { name: 'Sudah Kas', color: '#10b981' },
                      { name: 'Belum Kas', color: '#ef4444' },
                      { name: 'Sudah Sukarela', color: '#3b82f6' },
                      { name: 'Belum Sukarela', color: '#f59e0b' }
                    ].filter((_, index) => [
                      stats.paidKas, stats.unpaidKas, stats.donatedSukarela, stats.notDonatedSukarela
                    ][index] > 0).map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.color} />
                    ))}
                  </Pie>
                  <Tooltip 
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
              <div className="text-center p-3 bg-emerald-50 rounded-xl">
                <p className="text-sm font-medium text-emerald-700 mb-1">Sudah Kas</p>
                <p className="text-lg font-bold text-gray-800">{stats.paidKas}</p>
                <p className="text-xs text-emerald-600">{stats.paidPercentage}%</p>
              </div>
              <div className="text-center p-3 bg-red-50 rounded-xl">
                <p className="text-sm font-medium text-red-700 mb-1">Belum Kas</p>
                <p className="text-lg font-bold text-gray-800">{stats.unpaidKas}</p>
                <p className="text-xs text-red-600">{stats.unpaidPercentage}%</p>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-xl">
                <p className="text-sm font-medium text-blue-700 mb-1">Sudah Sukarela</p>
                <p className="text-lg font-bold text-gray-800">{stats.donatedSukarela}</p>
                <p className="text-xs text-blue-600">{stats.sukarelaPercentage}%</p>
              </div>
              <div className="text-center p-3 bg-amber-50 rounded-xl">
                <p className="text-sm font-medium text-amber-700 mb-1">Belum Sukarela</p>
                <p className="text-lg font-bold text-gray-800">{stats.notDonatedSukarela}</p>
                <p className="text-xs text-amber-600">{stats.notSukarelaPercentage}%</p>
              </div>
            </div>
          </div>
        </section>
