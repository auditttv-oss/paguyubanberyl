...
import { UserCheck, Clock, LogOut, Loader2, ShieldCheck, AlertCircle } from 'lucide-react';
import { supabase } from '../supabaseClient';
import { fetchTamu, createTamu, updateTamuKeluar } from '../services/dataService';
...
export const Keamanan = () => {
  ...
  const handleMasuk = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsSubmitting(true); // Cegah Double-Submission Satpam
    try {
    ...
    <div className="space-y-6 flex-1 flex flex-col h-full relative p-2 sm:p-4">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-500 text-xs">Pencatatan keluar masuk tamu, titip kartu identitas, dan tujuan kunjungan.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm flex flex-col lg:sticky lg:top-6">
          <h4 className="font-black text-slate-800 text-sm uppercase tracking-wider mb-4 border-b pb-3">Catat Tamu Masuk</h4>
          <form onSubmit={handleMasuk} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 outline-none transition-all disabled:bg-gray-100 disabled:cursor-not-allowed"
              />
            </div>
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 border border-slate-200 rounded-xl text-xs focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 outline-none transition-all font-bold text-emerald-900 disabled:bg-gray-100 disabled:cursor-not-allowed"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 border border-slate-200 bg-white rounded-xl text-xs focus:ring-2 focus:ring-emerald-200 focus:border-emerald-500 outline-none transition-all font-bold text-gray-700 disabled:bg-gray-100"
              >
            ...
            <button
              type="submit"
              disabled={isSubmitting}
              className="w-full bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-black py-3.5 rounded-xl uppercase tracking-widest transition-all shadow-md shadow-emerald-600/10 disabled:bg-gray-400 disabled:cursor-not-allowed"
            >
              {isSubmitting ? <Loader2 size={16} className="animate-spin mx-auto" /> : 'Catat Masuk'}
            </button>
          </form>
        </div>
              ...
              <table className="w-full text-left text-xs text-slate-600 min-w-[650px]">
                <thead className="bg-slate-50 text-[9px] uppercase tracking-wider text-slate-500 border-b border-slate-200">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-bold text-center">Status / Waktu Keluar</th>
                  </tr>
                </thead>
